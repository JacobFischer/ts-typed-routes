import { route as normalRoute, parameter, optionalParameter } from '../src';
import { reactRoute } from '../src/react-router';

const routeFunctions: typeof normalRoute[] = [normalRoute, reactRoute];

routeFunctions.forEach((route) => {
  describe(`${route.name}() base route functionality`, () => {
    it('exists', () => {
    ...
    describe('.segments', () => {
      it('has segments', () => {
        const segments = ['foo', 'bar', 'baz'] as const;
        const simple = route(...segments);
        expect(Array.isArray(simple.segments)).toBe(true);
        expect(simple.segments).toMatchObject(segments);
      });
    });
